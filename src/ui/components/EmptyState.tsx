...
export function EmptyState({ type, query, onRetry, onClear }: Props) {
  const content = {
    "no-results": {
      ...
      action: onClear && (
        <button onClick={onClear} style={style.btn}>
          Clear search
        </button>
      ),
    },
    ...
    "no-favorites": {
      icon: (
        <svg viewBox="0 0 40 40" width="36" height="36" fill="none">
          <path
            d="M20 32s-14-9-14-18a8 8 0 0116 0 8 8 0 0116 0c0 9-14 18-14 18z"
            stroke="currentColor"
            strokeWidth="2"
          />
        </svg>
  ...
  return (
    <div style={style.wrap}>
      <div style={style.icon}>{content.icon}</div>
      <p style={style.title}>{content.title}</p>
      <p style={style.hint}>{content.hint}</p>
      {content.action}
    </div>
  );
}
