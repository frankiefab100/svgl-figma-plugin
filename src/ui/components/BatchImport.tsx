...
export function BatchImport({ selected, onImport, onClear, onRemove }: Props) {
  ...
  function go() {
    onImport({
      logos: logos.map((l) => ({
        svgUrl: resolveLogoUrl(l.route, "light"),
        name: l.title,
      })),
      size,
      layout,
  ...
  return (
    <div style={style.wrap} className="animate-in">
      ...
      <div style={style.countRow}>
        <span style={style.badge}>{logos.length} selected</span>
        <button onClick={onClear} style={style.clearBtn}>
          Clear
        </button>
      </div>
      ...
      <div style={style.chips}>
        {logos.map((l) => (
          <div key={l.id} style={style.chip}>
            <img
              src={resolveLogoUrl(l.route, "light")}
              alt={l.title}
              style={style.chipImg}
            />
            <span style={style.chipName}>{l.title}</span>
            <button onClick={() => onRemove(l.id)} style={style.chipX}>
      ...
      <div style={style.section}>
        <span style={style.sLabel}>Import Settings</span>
      ...
      <p style={style.disclaimer}>
        Logos are imported as editable SVGs.
        <br />
        Please verify usage rights before commercial use.
      </p>
    </div>
  );
}
