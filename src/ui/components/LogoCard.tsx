...
export function LogoCard({ logo, selected, batchMode, batchSelected, onSelect, onImport, onToggleBatch }: Props) {
  const [hovered, setHovered] = useState(false);
  const [fav, setFav] = useState(() => isFavorite(logo.id));
  const [imgErr, setImgErr] = useState(false);

  const url = resolveLogoUrl(logo.route, "light");

  function handleClick() {
    if (batchMode) { onToggleBatch(); return; }
    onSelect();
  }

  function handleFav(e: React.MouseEvent) {
    e.stopPropagation();
    setFav(toggleFavorite(logo.id));
  }

  const cardStyle: React.CSSProperties = {
    ...s.card,
    background: selected || batchSelected ? "var(--surface-hover)" : hovered ? "var(--surface-hover)" : "transparent",
  ...
  return (
    <div
      ...
      onClick={handleClick}
      onDoubleClick={() => { if (!batchMode) onImport(); }}
    ...
    >
      ...
      <div style={s.imgWrap}>
        {imgErr
          ...
          : <img src={url} alt={logo.title} style={s.img} loading="lazy" onError={() => setImgErr(true)} />
        }
      </div>
      ...
      >
        <svg viewBox="0 0 12 12" width="11" height="11" fill={fav ? "var(--danger)" : "none"}>
          <path
...
const s: Record<string, React.CSSProperties> = {
  card: {
    ...
    alignItems: "center", gap: 5, padding: "10px 2px 8px",
    borderRadius: "var(--radius)", cursor: "pointer",
    userSelect: "none", transition: `all var(--dur) var(--ease)`,
    minWidth: 0, overflow: "hidden", width: "100%",
  },
  imgWrap: {
    width: "100%", maxWidth: 44, height: 44, display: "flex", alignItems: "center",
    justifyContent: "center", padding: 4, borderRadius: 6, flexShrink: 0,
  },
  img: { maxWidth: 36, maxHeight: 36, objectFit: "contain", display: "block" },
  fallback: {
    width: 36, height: 36, borderRadius: 4, background: "var(--surface-active)",
  ...
  name: {
    ...
    whiteSpace: "nowrap", maxWidth: "100%", padding: "0 2px",
  },
  importBtn: {
    position: "absolute", bottom: 0, left: 0, right: 0,
    background: "var(--accent)", color: "white", border: "none",
    borderRadius: "0 0 var(--radius) var(--radius)", fontSize: 10,
    fontWeight: 600, fontFamily: "var(--font)", padding: "4px 0",
  ...
  variantDot: {
    position: "absolute", top: 4, left: 4, width: 5, height: 5,
    borderRadius: "50%", background: "var(--text3)",
  },
