...
import { addRecentLogo, getRecentLogos, getFavoriteIds, clearRecentLogos } from "./lib/storage";
...
export default function App() {
  const {
    logos, allLogos, categories,
    query, setQuery,
    activeCategory, setActiveCategory,
    isLoading, isEmpty, error, retry,
  } = useLogos();
  ...
  // Refresh favIds whenever the window gets focus (user may have toggled in another card)
  useEffect(() => {
    const refresh = () => setFavIds(getFavoriteIds());
    window.addEventListener("focus", refresh);
    // Also poll every 500ms while plugin is open — cards toggle favorites locally
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key === "Escape") {
        ...
        if (batchMode) { setBatchMode(false); setBatchMap(new Map()); }
      }
    }
  ...
  function openPreview(logo: SVGLogo) {
    if (batchMode) { toggleBatch(logo); return; }
    setPreview(logo);
    setView("preview");
  }
  ...
  function quickImport(logo: SVGLogo) {
    ...
    sendToPlugin({
      ...
      payload: {
        ...
        name: logo.title, size: 48,
        createComponent: false, placement: "cursor",
      },
    });
  }
  ...
  function importFromPreview(opts: {
    ...
    if (preview) { addRecentLogo(preview); setRecent(getRecentLogos()); }
    setImporting(true);
  ...
  function importBatch(opts: {
    ...
    setImporting(true);
    Array.from(batchMap.values()).forEach(addRecentLogo);
  ...
  const displayed: SVGLogo[] = (() => {
    ...
    if (filter === "favorites") return allLogos.filter((l) => favIds.includes(l.id));
    return logos;
  })();
  ...
  if (view === "preview" && preview) return (
    <div style={lay.root}>
      <LogoPreview logo={preview} onBack={() => setView("main")} onImport={importFromPreview} />
      <ToastContainer />
    </div>
  );
  ...
  if (view === "batch") return (
    <div style={lay.root}>
      <BatchImport
        ...
        onRemove={(id) => setBatchMap((m) => { const n = new Map(m); n.delete(id); return n; })}
      />
      <ToastContainer />
    </div>
  );
    ...
    <div style={lay.root}>
      ...
      <div style={lay.header}>
        ...
        <div style={lay.actions}>
          ...
          <IconBtn
            active={batchMode}
            title={batchMode ? (batchMap.size > 0 ? `View ${batchMap.size}` : "Exit batch") : "Batch import"}
            onClick={() => batchMode && batchMap.size > 0 ? setView("batch") : setBatchMode((b) => !b)}
          >
            {batchMode && batchMap.size > 0
              ? <span style={{ fontSize: 10, fontWeight: 700, color: "var(--accent)" }}>{batchMap.size}</span>
              : <MultipleIcon />}
          </IconBtn>
          ...
          {/* Close */}
          <IconBtn title="Close" onClick={() => sendToPlugin({ type: "CLOSE" })}>
      ...
      {batchMode && (
        <div style={lay.batchBanner}>
          ...
          <button onClick={() => { setBatchMode(false); setBatchMap(new Map()); }} style={lay.batchCancel}>
            Cancel
          </button>
        </div>
      )}
      ...
      <div style={lay.padX}>
        <SearchBar
          value={query}
          onChange={(v) => { setQuery(v); setFilter("all"); }}
          autoFocus
        />
      </div>
      ...
      <div style={lay.padX}>
        <CategoryTabs
          ...
          onChange={(c) => { setActiveCategory(c); setFilter("all"); setQuery(""); }}
        />
      </div>
      ...
      <div style={lay.filterRow}>
        {(["all", "recent", "favorites"] as Filter[]).map((f) => (
          <button
            key={f}
            onClick={() => { setFilter(f); if (f !== "all") { setQuery(""); setActiveCategory("All"); } }}
            style={{ ...lay.filterBtn, ...(filter === f ? lay.filterOn : {}) }}
          >
            {f === "all" ? "All Logos" : f === "recent" ? "Recent" : "Favorites"}
          </button>
        ))}

        {/* refresh favorites badge */}
        {filter === "favorites" && (
          <button onClick={() => setFavIds(getFavoriteIds())} style={lay.refreshBtn}>↻</button>
        )}

        {filter === "recent" && recent.length > 0 && (
          <button
            onClick={() => { clearRecentLogos(); setRecent([]); }}
            style={lay.clearBtn}
          >
            Clear
          </button>
      ...
      <div style={lay.scroll}>
        ...
        ) : (isEmpty || (filter !== "all" && displayed.length === 0 && !isLoading)) ? (
          <EmptyState type="no-results" query={query} onClear={() => setQuery("")} />
        ) : (
          <LogoGrid
            logos={displayed}
      ...
      <div style={lay.footer}>
        <span>
          <a href="https://svgl.app" style={lay.fLink}>View source</a>
          {" · "}
          <a href="https://github.com/pheralb/svgl/issues" style={lay.fLink}>Report issue</a>
        </span>
        <span>v1.0.0</span>
      </div>
...
function MultipleIcon() {
  return (
    <svg viewBox="0 0 16 16" width="13" height="13" fill="none">
      <rect x="5" y="1" width="10" height="10" rx="1.5" stroke="currentColor" strokeWidth="1.3" strokeDasharray="2 1" opacity="0.7" />
      <rect x="1" y="5" width="10" height="10" rx="1.5" stroke="currentColor" strokeWidth="1.3" fill="none" />
      <path d="M4 10h4M6 8v4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
    </svg>
  );
}
...
const lay: Record<string, React.CSSProperties> = {
  ...
  brandName: { fontSize: 13, fontWeight: 700, color: "var(--text)", lineHeight: "16px" },
  ...
  batchGo: {
    marginLeft: "auto", border: "none", background: "var(--accent)",
    color: "white", fontSize: 10, fontWeight: 600,
    fontFamily: "var(--font)", padding: "2px 8px", borderRadius: 100, cursor: "pointer",
  },
  batchCancel: {
    border: "none", background: "transparent", color: "var(--accent)",
    fontSize: 10, fontFamily: "var(--font)", cursor: "pointer", padding: "2px 4px",
  },
  ...
  filterBtn: {
    ...
    padding: "3px 4px", borderBottom: "2px solid transparent",
  },
  filterOn: {
    color: "var(--text)", borderBottomColor: "var(--accent)", fontWeight: 600,
  },
  refreshBtn: {
    border: "none", background: "transparent", color: "var(--text3)",
  ...
  clearBtn: {
    ...
    color: "var(--text3)", fontSize: 10, fontFamily: "var(--font)", cursor: "pointer",
  },
  count: { padding: "4px 12px 0", fontSize: 10, color: "var(--text3)", flexShrink: 0 },
  scroll: { flex: 1, overflowY: "auto", overflowX: "hidden", padding: "0 10px" },
  ...
  overlay: {
    ...
    background: "rgba(0,0,0,0.15)", zIndex: 999, pointerEvents: "none",
  },
  spinner: {
    width: 20, height: 20,
